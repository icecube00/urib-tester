...
var saveAs = saveAs
	...
	,
		...
		view.onunload = function onunload(){process_deletion_queue();};
...
var saveTextAs = function (textContent, fileName, charset) {
		...
		if (saveAs && Blob) {
			...
			if (fileName.endsWithAny('.htm', '.html')) {
				...
				doc.body.innerHTML = '\r\n' + textContent + '\r\n';
					...
					if (!fileName.endsWithAny('.txt', '.xml'))
						fileName += '.txt';
					doc.write(textContent);
					doc.close();
				var retValue = doc.execCommand('SaveAs', 'MSOCMDEXECOPT_DONTPRONMTUSER', fileName);
				saveTxtWindow.close();
				return retValue;
			}
		}
