...
(function () {
	...
	(),
	b = RegExp.prototype.sticky !== undefined,
	i = {};
	...
	RegExp.prototype.exec = function (t) {
		...
		var r = m.exec.apply(this, arguments),
		q,
		p;
		if (r) {
			...
			p = m.replace.call(u, r, function () {
					...
					return m.replace.call(s, c, function (x, w, A) {
						if (w) {
							...
							case "'":
								return v[v.length - 1].slice(v[v.length - 2] + v[0].length);
							default:
								var y = "";
								w = +w;
								if (!w) {
...
var SyntaxHighlighter = function () {
	...
	var l = {
		...
		highlight : function (P, N) {
			var K = this.findElements(P, N),
			Q = "innerHTML",
			U = null,
			S = l.config;
			if (K.length === 0) {
	...
	function b(L) {
		var K = /(.*)((&gt;|&lt;).*)/;
		return L.replace(l.regexLib.url, function (M) {
			var O = "",
			N = null;
			if (N = K.exec(M)) {
				M = N[1];
	...
	l.Highlighter.prototype = {
		...
		forHtmlScript : function (K) {
			this.htmlScript = {
				...
				code : new XRegExp("(?<left>" + K.left.source + ")(?<code>.*?)(?<right>" + K.right.source + ")", "sgi")
			}
		}
	};
	return l
